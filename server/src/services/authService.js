...
export default {
  // Add this to authService.js exports
  async resetPassword(userId, newPassword) {
    const password_hash = await this.hashPassword(newPassword.trim());

    const { rowCount } = await query(
      `UPDATE employee_auth SET password_hash = $1 WHERE id = $2`,
      [password_hash, userId],
    );

    if (rowCount === 0) throw new Error("کاربر یافت نشد.");
  ...
  async login(username, password) {
    ...
    await query(`INSERT INTO employee_login(employee_id) VALUES ($1)`, [
      user.id,
    ]);
