...
export default function Services() {
    ...
    const {selectedService , setSelectedService , clearSelectedService} = useSelectedServiceStore()
                                        ...
                                        ? (
                                            services.map((service) => (
                                                <div key={service.service_id}
                                                    ...
                                                    <div
                                                        className='w-full h-full px-3 py-2 flex items-center justify-between'>
                                                        <div
                                                            className='flex items-center gap-1'>{icons.find(icon => icon.label === service.icon)?.component &&
                                                            React.cloneElement(
                                                                icons.find(icon => icon.label === service.icon)!.component,
                                                                {size: 31} // Set your desired size here
                                                            )
                                                        }
                                                            <p className='font-medium bold truncate max-w-32 '>{service.name}</p>
                                                        </div>

                                                        <div className='flex h-full items-end justify-center'>
                                                            <p className='font-semibold'>{formatToPersianCurrency(service.amount)}
                                                                <span className='text-xs font-light'>ریال</span></p>
                                                        </div>
                                                    </div>
                                                    <div className="w-full flex items-center border-t rounded-b-md"
                                                         dir="rtl">
                                                        {selectedService ? (
                                                            <button
                                                                type='button'
